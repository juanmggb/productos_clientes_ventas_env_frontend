...
// Componente ////////////////////////////////////////////////////
const VentaRenglon = ({ venta, mostrarDetallesVenta, manejarVentaDetalles }) => {
	...
	return (
		<VentaRenglonEstilo onClick={() => llamarMostrarDetallesVenta(venta)}>
			<div><span>{venta.id}</span></div>
            <div><span>{venta.CLIENTE ? venta.cliente_nombre : 'No Disponible'}</span></div>
            <div><span>{fecha}</span></div>
            <div><span>{hora}</span></div>
            <div><span>{venta.TIPO_VENTA}</span></div>
            <div><span>{venta.VENDEDOR}</span></div>
            <div><span>{venta.TIPO_PAGO}</span></div>
            <div><span>{venta.STATUS}</span></div>
            <div style={{display: 'flex',justifyContent: 'center'}}><span><Modificar onClick={() => manejarVentaDetalles(venta.id)}>
                <i className="fa-solid fa-circle-info"/></Modificar></span></div>
